...
namespace CapaDatos
{
    public class CrudAngularNetAppi
    {
        ...
        public bool DeleteEstudiante(ModeloEstudiante EliminarRegistro)
        {
            try
            {
                using var db = conexion.ObtenerConexion();
                ...
                string checkFK = "SELECT COUNT(*) FROM Nota WHERE idEstudiante = @id";
                using (SqlCommand checkSql = new SqlCommand(checkFK, db))
                {
                    try
                    {
                        ...
                        if (fkCount > 0)
                        {
                            Debug.WriteLine("[****].[INFO].[DeleteEstudiante] No se puede eliminar, tiene " + fkCount + " notas asociadas.");
                            return false;
                        }
                    }
                    catch (SqlException e) when (e.Number == 547)
                    {
                        Debug.WriteLine("[****].[INFO].[DeleteEstudiante] No se puede eliminar el estudiante, tiene notas asociadas.");
                    }
                }
            ...
            {
                Debug.WriteLine("[****].[ERROR].[Capa Datos].[DeleteEstudiante].[DeleteOrigen] " + e.Message);
                throw new Exception("ERROR [Capa Datos].[DeleteEstudiante].[DeleteOrigen] " + e.Message);
            }
        }
